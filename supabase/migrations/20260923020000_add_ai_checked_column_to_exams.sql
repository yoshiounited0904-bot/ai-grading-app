-- ==============================================================================
-- Add is_ai_checked and ai_checked_at columns to exams table & update admin_save_exam RPC
-- ==============================================================================

ALTER TABLE public.exams ADD COLUMN IF NOT EXISTS is_ai_checked BOOLEAN DEFAULT false NOT NULL;
ALTER TABLE public.exams ADD COLUMN IF NOT EXISTS ai_checked_at TIMESTAMPTZ;
...
BEGIN
  ...
  INSERT INTO public.exams (
    ...
    COALESCE((p_exam->>'is_ai_checked')::boolean, false),
    CASE 
      WHEN (p_exam->>'is_ai_checked')::boolean IS TRUE THEN timezone('utc'::text, now())
      ELSE NULL
    END,
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    is_ai_checked = COALESCE(EXCLUDED.is_ai_checked, exams.is_ai_checked),
    ai_checked_at = CASE
      WHEN EXCLUDED.is_ai_checked IS TRUE AND exams.is_ai_checked IS NOT TRUE THEN timezone('utc'::text, now())
      WHEN EXCLUDED.is_ai_checked IS FALSE THEN NULL
      ELSE exams.ai_checked_at
    END,
